...
BEGIN
	DECLARE @silver_load_start_time DATETIME, @silver_load_end_time DATETIME
	...
	*/
	...
	BEGIN TRY
		...
		PRINT '>> Succesfull, load duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds'
			PRINT'----------------------------------'
		/*
		==========================
    ...
    PRINT 'FULL SILVER LAYER EXCECUTION TIME: ' 
        + CAST(DATEDIFF(second, @silver_load_start_time, @silver_load_end_time) AS NVARCHAR(10)) 
        + ' seconds';
END
